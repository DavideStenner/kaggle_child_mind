...
class MLPTrainer(ModelTrain, MLPInit):
    ...
    def train_target(self, fold_: int, model_type: str) -> None:
        ...
        model = Standalone_RealMLP_TD_S_Regressor()
        
        history_loss, history_param = model.fit(train_matrix, test_matrix)
        setattr(
    ...
    def get_dataset(self, fold_: int) -> Tuple[Tuple[np.ndarray]]:
        ...
        train_matrix = (
            train_filtered.select(self.feature_list).collect().to_pandas(),
            train_filtered.select(self.target_col).collect().to_pandas(),
        )
        test_matrix = (
            test_filtered.select(self.feature_list).collect().to_pandas(),
            test_filtered.select(self.target_col).collect().to_pandas(),
        )
    ...
    def save_model(self, target: str)->None:     
        ...
        self.save_pickle_model_param_list(
            getattr(
