...
class TabnetInit(ModelInit):
    ...
    def initialize_model_utils(self) -> None:
        ...
        self.mean_list: list[dict[str, float]] = []
    ...
    def set_postprocess_utils(self) -> None:
        ...
        # )
        treshold_base: list[float] = [0.5, 1.5, 2]
    ...
    def save_pickle_model_list(self, model_list: list[TabNetRegressor], model_type: str) -> None:
        with open(
            os.path.join(
    ...
    def load_pickle_model_list(self, model_type: str) -> list[TabNetRegressor]:
        with open(
            os.path.join(
                self.experiment_path_dict['model'].format(model_type=model_type),
                self.model_file_name_dict['model_pickle_list'][model_type]
    ...
    def save_mean_list(self, mean_list: list[dict[str, float]], model_type: str) -> None:
        with open(
            os.path.join(
                self.experiment_path_dict['model'].format(model_type=model_type),
                self.model_file_name_dict['mean_vector'][model_type]
            ), 'wb'
        ) as file:
            pickle.dump(mean_list, file)
    
    def load_mean_list(self, model_type: str) -> list[dict[str, float]]:
        with open(
            os.path.join(
                self.experiment_path_dict['model'].format(model_type=model_type),
                self.model_file_name_dict['mean_vector'][model_type]
