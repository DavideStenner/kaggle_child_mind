...
from src.model.tabnet.model import TabNetDumpModel
from src.model.metric.official_metric import QKappaTabnet
...
class TabnetTrainer(ModelTrain, TabnetInit):
    ...
    def train_target(self, fold_: int, model_type: str) -> None:
        ...
        model = TabNetRegressor(**self.params_tabnet['model_architecture'])
        dump_model_list: TabNetDumpModel = TabNetDumpModel()
        
        model.fit(
            X_train=train_matrix[0], y_train=train_matrix[1],
            eval_set=[test_matrix],
            eval_name=['valid'],
            eval_metric=['rmse', QKappaTabnet],
            max_epochs=self.params_tabnet['epoch'],
            patience=0,
            batch_size=1024,
            virtual_batch_size=128,
            num_workers=0,
            drop_last=False,
            callbacks=[dump_model_list]
        )
        setattr(
        ...
        setattr(
            ...
            (
                ...
                [{key_: list_ for key_, list_ in model.history.history.items() if 'valid' in key_}]
            )
        )
    ...
    def __check_validity_train_dataset(self) -> None:
        ...
        accepted_dtypes_col = feature_dataset.select(pl.all()).select(pl.col(NUMERIC_POLARS_DTYPES)).collect_schema().names()
    ...
    def get_dataset(self, fold_: int) -> Tuple[Tuple[np.ndarray]]:
        ...
        train_filtered = (
            ...
            .with_columns(
                [
                    pl.col(col).fill_null((pl.col(col).max()+1))
        ...
        #train mean
        train_filtered_mean_vector = {
            col: train_filtered.select(pl.col(col).mean()).collect().item()
            for col in self.feature_list if col not in self.categorical_col_list
        }
        self.mean_list.append(train_filtered_mean_vector)
            ...
            .with_columns(
                [
                    (
                        ...
                        .then(pl.lit(train_filtered_mean_vector[col]))
    ...
    def train(self) -> None:
        ...
        for model_type in self.model_used:
            ...
            self.save_mean_list(mean_list=self.mean_list, model_type=model_type)
            self.shrink_to_best_model(target=model_type)
    ...
    def shrink_to_best_model(self, target: str)->None:
        metric_to_check: str = f"valid_{self.model_metric_used[target]['label']}"
        metric_to_max: bool = self.model_metric_used[target]['maximize']
        
        progress_list: list[dict[str, float]] = [
            progress_[metric_to_check]
            for progress_ in getattr(
                self, f'progress_{target}_list'
            )
        ]
        progress_df: dict = pd.DataFrame(
            {
                f"{metric_to_check}_fold_{i}": progress_list[i]
                for i in range(self.n_fold)
            }
        )
        progress_df['time'] = range(progress_df.shape[0])
        progress_df[f"average"] = progress_df.loc[
            :, [metric_to_check in x for x in progress_df.columns]
        ].mean(axis =1)

        if metric_to_max:
            best_epoch = int(progress_df[f"average"].argmax())
        else:
            best_epoch = int(progress_df[f"average"].argmin())
            
        self.training_logger.info(f'Best epoch to {best_epoch}. Shrinking all model to this epoch')
        
        best_model_param = [
            pickle.loads(params_[best_epoch])
            for params_ in getattr(
                self, f'model_param_{target}_list'
            )
        ]
        model_list: list[TabNetRegressor] = getattr(
            self, f'model_{target}_list'
        )
        for n_fold, model_ in enumerate(model_list):
            model_.network.load_state_dict(best_model_param[n_fold])
            model_.network.eval()
            model_list[n_fold] = model_
            
        setattr(
            self, f'model_{target}_list', model_list
        )
        setattr(
            self, f'model_param_{target}_list', None
        )
