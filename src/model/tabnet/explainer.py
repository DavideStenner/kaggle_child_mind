...
class TabnetExplainer(TabnetInit):       
    ...
    def __evaluate_single_model(self, model_type: str) -> None:
        ...
        list_metric = [metric_.replace('valid_', '') for metric_ in progress_list[0].keys()]

        for metric_ in list_metric:
            progress_dict.update(
                {
                    f"{metric_}_fold_{i}": progress_list[i]['valid_' + metric_]
    ...
    def __get_list_of_oof_dataset(self) -> list[Tuple[pd.DataFrame, pd.DataFrame]]:
        ...
        for fold_ in range(self.n_fold):
                ...
                .with_columns(
                    ...
                    [
                        (
                            ...
                            .then(pl.lit(self.mean_list[fold_][col]))
            ...
            test_target = (
                ...
                .to_pandas()
                .to_numpy(self.target_precision)
            )
    ...
    def oof_get_best_treshold(self) -> None:
        ...
        for model_type in self.model_used:
            self.mean_list = self.load_mean_list(model_type=model_type)
            dataset_list: list[Tuple[pd.DataFrame, pd.DataFrame]] = self.__get_list_of_oof_dataset()
            model_list: list[TabNetRegressor] = self.load_pickle_model_list(
                model_type=model_type, 
            )
