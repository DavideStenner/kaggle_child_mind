...
class CtbInit(ModelInit):
    def __init__(self, 
        ...
        self.model_metric_used: list[str] = {
            target_: {'label': 'QuadraticKappa', 'maximize': True}
    ...
    def save_pickle_model_list(self, model_list: list[cb.CatBoost], model_type: str) -> None:
        with open(
            os.path.join(
                self.experiment_model_path.format(model_type=model_type),
                'model',
    ...
    def load_pickle_model_list(self, model_type: str) -> list[cb.CatBoost]:
        with open(
            os.path.join(
    ...
    def load_model_list(self, model_type: str, file_name: str) -> list[cb.CatBoost]:
